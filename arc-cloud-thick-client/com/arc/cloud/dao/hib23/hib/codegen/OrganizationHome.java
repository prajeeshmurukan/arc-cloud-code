...
public class OrganizationHome {
	...
	public Organization merge(Organization detachedInstance) {
		...
		try {
			Organization result = (Organization) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Organization findById(long id) {
		...
		try {
			Organization instance = (Organization) sessionFactory.getCurrentSession()
					.get("com.arc.cloud.dao.hib23.hib.codegen.Organization", id);
			if (instance == null) {
	...
	public List<Organization> findByExample(Organization instance) {
		...
		try {
			List<Organization> results = (List<Organization>) sessionFactory.getCurrentSession()
					.createCriteria("com.arc.cloud.dao.hib23.hib.codegen.Organization").add(create(instance)).list();
