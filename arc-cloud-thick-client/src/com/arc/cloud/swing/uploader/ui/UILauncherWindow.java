...
public class UILauncherWindow extends JFrame {
	static Customers custs = null;
	static String restURL = "http://localhost/arc-cloud/api/";
	...
	public UILauncherWindow() {
		...
		btnUpload.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				Thread thread = null;
				 ...
				 thread = new Thread(new ValidateUploadThreadUpload(table));
				 thread.start();
		...
		lblPleaseEnterOrganization.setVisible(false);
		JLabel lblPleaseEnterUser = new JLabel("Please enter E-mail");
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if ((textField_2.getText().trim().length() > 0) && (textField_3.getText().trim().length() > 0)
					...
					Customers cust = UIDBUtil.login( textField_3.getText(),textField_4.getText());
					boolean isvalidUser = false;
					if(cust != null){
						isvalidUser = true;
						UILauncherWindow.custs = cust;
					}
					if (isvalidUser) {
		...
		textField_3.setText("prajeesh.ms@gmail.com");
...
class ValidateUploadThreadUpload implements Runnable{
	...
	public void run() {
		...
		DefaultTableModel pModal= (DefaultTableModel)table_1.getModel();
		pModal.addColumn("Validate Result");
		table_1.getColumnModel().getColumn(11).setCellRenderer(new StatusColumnCellRenderer());
		// Start validating 
		...
		for(int i=0;i<noOfRows;i++){
			// processing per row
			Object dataId = table_1.getModel().getValueAt(i, 0);
			Object docObjId = table_1.getModel().getValueAt(i, 1);
			Object project = table_1.getModel().getValueAt(i, 2);
			Object dataTax1 = table_1.getModel().getValueAt(i, 3);
			Object dataTax2 = table_1.getModel().getValueAt(i, 4);
			Object docName = table_1.getModel().getValueAt(i, 5);
			Object docTitle = table_1.getModel().getValueAt(i, 6);
			Object createDt = table_1.getModel().getValueAt(i, 7);
			Object keyWords = table_1.getModel().getValueAt(i, 8);
			Object metadata = table_1.getModel().getValueAt(i, 9);
			Object orgVal= table_1.getModel().getValueAt(i, 10);
			 final FileDataBodyPart filePart = new FileDataBodyPart("file", new File(docName.toString()));
			    FormDataMultiPart formDataMultiPart = new FormDataMultiPart();
			    int userId =UILauncherWindow.custs.getId();
			    String orgId = String.valueOf(UILauncherWindow.custs.getOrganization().getId());
			    System.out.println("ValidateUploadThreadUpload.run() user and org id = " + userId +orgId );
			    formDataMultiPart = formDataMultiPart
			    .field("objID", docObjId.toString())
			    .field("userId", String.valueOf(userId))
			    .field("orgId", orgId.toString())
			    .field("projectName", project.toString())
			    .field("tax1", dataTax1.toString())
			    .field("tax2", dataTax2.toString())
			    .field("documentName", docName.toString())
			    .field("documentTitle", docTitle.toString())
			    .field("documentcrDt", createDt.toString())
			    .field("documentKeyWords", keyWords.toString())
			    .field("documentMetadata", metadata.toString())
			    .field("documentOrg", orgVal.toString());

			    
			    final FormDataMultiPart multipart = (FormDataMultiPart) formDataMultiPart.field("foo", "bar").bodyPart(filePart);
			      
			    WebTarget target = client.target(UILauncherWindow.restURL).path("archive/upload");
			    final Response response = target.request().post(Entity.entity(multipart, multipart.getMediaType()));
			     System.out.println("FileUploadClient.uploadMultipart() " + response.getStatus());
				    //Use response object to verify upload success
			     if(response.getStatus() == 200){
			    	 table_1.getModel().setValueAt(new String("Upload  Successful") , i, 11);
					 table_1.changeSelection(i, 11, false, false);
			     }else{
			    	 table_1.getModel().setValueAt(new String("Upload Not Successful") + response.getStatus(), i, 11);
			            table_1.changeSelection(i, 11, false, false);
			     }
